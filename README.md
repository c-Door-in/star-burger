...
[Пример рабочего сайта](https://basni.online)
...
В каталоге проекта перейдите в каталог `backend` и создайте виртуальное окружение:
```sh
python -m venv venv
```
...
Перейдите в папку `frontend` в каталоге проекта и установите пакеты Node.js:
...
- `YANDEX_GEO_APIKEY` - ключ от API Яндекс геокодер. Нужен для расчета расстояния до ближайшего ресторана. Для этого небходимо зарегистрироваться 
в [разделе разработчиков Яндекс](https://developer.tech.yandex.ru/) 
и подключить API интерфейс "JavaScript API и HTTP Геокодер".
- `DJANGO_ROLLBAR_TOKEN` - ключ от системы логгирования Rollbar. Нужен для отслеживания логов 
с сервера. Необходимо зарегистрироваться на [Rollbar](https://rollbar.com/). Создать проект и скопировать ключ доступа `post_server_item`, находящийся среди активных токенов.
- `ROLLBAR_ENVIRONMENT` - тип среды для указания при логгировании. Установите 'production'.
- `POSTGRES_HOST` - место расположения базы данных (по-умолчанию `db`).
- `POSTGRES_PORT` - порт доступа к базе данных (по-умлочанию `5432`)
...
Перед запуском присвойте переменным окружения значения в файле `.env` (смотри объяснение переменных в разделе prod-версии). 
Установите дебаг-режим: `DEBUG=True`  
Postgresql устанавливается внутри контейнера, данные для него можно указать любые в файле .env. Переменные `POSTGRES_HOST` и 
`POSTGRES_PORT` не указывать.
Убедитесь, что Докер установлен и запущен: `docker --version`
Перейдите в папку проекта и запустите команду:
```sh
docker compose up --build
```
После поднятия сайт будет доступен по адресу [http://localhost:8080](http://localhost:8080)
После внесения изменений выполните последовательно:
```sh
docker compose down
docker compose up --build
```
